...
class AddDefaultScriptUseCase(
    ...
    override suspend fun run(params: Params): Either<Failure, None> {
        ...
        val script = TetroidScript(
            ...
            fileName = defaultScript.fileName,
        ...
        return if (scriptsManager.insertScript(script)) {
            saveScriptTextToFile(script)
        } else {
            Failure.Database.Insert.toLeft()
        }
    }

    private suspend fun saveScriptTextToFile(script: TetroidScript): Either<Failure, None> {
        val scriptPath = "scripts/${script.fileName}"
