...
class RecordsInteractor(
    ...
    val mTagsParser: ITagsParser,
    val mCryptInteractor: EncryptionInteractor,
    val mDataInteractor: DataInteractor,
    val mInteractionInteractor: InteractionInteractor
) {
...
//        String dirPath = getPathToRecordFolderInBase(record);
        val dirPath = RecordsManager.getPathToRecordFolder(context, record)
        if (RecordsManager.checkRecordFolder(context, dirPath, true, duration) <= 0) {
            return null
        }
        val path = dirPath + DataManager.SEPAR + record.fileName
        val uri: Uri
        uri = try {
            Uri.parse(path)
        } catch (ex: Exception) {
    ...
    fun getRecordTextDecrypted(context: Context?, record: TetroidRecord): String? {
        ...
        val html = RecordsManager.getRecordHtmlTextDecrypted(context, record, -1)
        if (html != null) {
            try {
...
//        String dirPath = (record.isTemp()) ? getPathToRecordFolderInTrash(context, record) : getPathToRecordFolderInBase(record);
    ...
    fun checkRecordFolder(context: Context, dirPath: String?, isCreate: Boolean, duration: Int): Int {
        ...
        try {
            if (!folder.exists()) {
                if (isCreate) {
                    LogManager.log(
                        context, String.format(Locale.getDefault(), context.getString(R.string.log_create_record_dir), dirPath),
     ...
     * @return
     */
    protected fun cloneRecordToNode(
        context: Context, srcRecord: TetroidRecord?, node: TetroidNode,
        isCutted: Boolean, breakOnFSErrors: Boolean
    ): TetroidRecord? {
        if (srcRecord == null) return null
        ...
        val id = if (isCutted) srcRecord.id else DataManager.createUniqueId()
        val dirName = if (isCutted) srcRecord.dirName else DataManager.createUniqueId()
        ...
        mTagsParser.parseRecordTags(record, tagsString)
        val errorRes = if (breakOnFSErrors) null else record
        var srcDirPath: String? = null
        var srcDir: File? = null
        // проверяем существование каталога записи
        srcDirPath = if (isCutted) {
            RecordsManager.getPathToRecordFolderInTrash(context, srcRecord)
        } else {
            RecordsManager.getPathToRecordFolderInBase(srcRecord)
        }
        val dirRes = RecordsManager.checkRecordFolder(context, srcDirPath, false)
        srcDir = if (dirRes > 0) {
            File(srcDirPath)
        } else {
            return errorRes
        }
        val destDirPath = RecordsManager.getPathToRecordFolderInBase(record)
        val destDir = File(destDirPath)
        if (isCutted) {
            // вырезаем уникальную приставку в имени каталога
            val dirNameInBase = srcRecord.dirName.substring(DataManager.PREFIX_DATE_TIME_FORMAT.length + 1)
            // перемещаем каталог записи
            val res = moveRecordFolder(context, record, srcDirPath, DataManager.getStoragePathBase(), dirNameInBase)
            if (res < 0) {
                return errorRes
            }
        } else {
                ...
                if (FileUtils.copyDirRecursive(srcDir, destDir)) {
                    LogManager.log(
                        context, String.format(
                            context.getString(R.string.log_copy_record_dir_mask),
                            destDirPath
                        ), ILogger.Types.ERROR
                    )
                    ...
                    LogManager.log(
                        context, String.format(
                            context.getString(R.string.log_error_copy_record_dir_mask),
...
//        if (!cryptOrDecryptFile(recordFile, srcRecord.isCrypted(), crypted) && breakOnFSErrors) {
        return if (!mCryptInteractor.cryptRecordFiles(context, record, srcRecord.isCrypted, crypted) && breakOnFSErrors) {
            errorRes
        } else record
    }
    ...
    private fun cloneAttachesToRecord(srcRecord: TetroidRecord, destRecord: TetroidRecord, isCutted: Boolean) {
        if (srcRecord.attachedFilesCount > 0) {
            ...
            for (srcAttach in srcRecord.attachedFiles) {
                // генерируем уникальные идентификаторы, если запись копируется
                val id = if (isCutted) srcAttach.id else DataManager.createUniqueId()
    ...
    private fun renameRecordAttaches(context: Context, srcRecord: TetroidRecord, destRecord: TetroidRecord) {
        for (i in 0 until srcRecord.attachedFilesCount) {
            ...
            // переименовываем
            val destPath = RecordsManager.getPathToRecordFolderInBase(destRecord)
            val srcFile = File(destPath, srcFileIdName)
            val destFile = File(destPath, destFileIdName)
            if (srcFile.renameTo(destFile)) {
                val to = DataManager.getStringTo(context, destFile.absolutePath)
...
//                        destFile.getAbsolutePath()), LogManager.Types.DEBUG);
                ...
                val fromTo = DataManager.getStringFromTo(context, srcFile.absolutePath, destFile.name)
...
//                        srcFile.getAbsolutePath(), destFile.getName()), LogManager.Types.ERROR);
     ...
     */
    fun createRecord(
        context: Context, name: String?, tagsString: String?, author: String?, url: String?,
        node: TetroidNode?, isFavor: Boolean
    ): TetroidRecord? {
        ...
        val id = DataManager.createUniqueId()
        val dirName = DataManager.createUniqueId()
        val crypted = node.isCrypted
        val record = TetroidRecord(
            crypted, id,
            encryptField(crypted, name),
        ...
        val dirPath = RecordsManager.getPathToRecordFolderInBase(record)
        if (RecordsManager.checkRecordFolder(context, dirPath, true) <= 0) {
            return null
        }
        val dir = File(dirPath)
        // создаем файл записи (пустой)
        val filePath = dirPath + DataManager.SEPAR + record.fileName
        val fileUri: Uri
        fileUri = try {
            Uri.parse(filePath)
        } catch (ex: java.lang.Exception) {
     ...
     * @return
     */
    fun createTempRecord(context: Context, name: String?, url: String?, text: String?): TetroidRecord? {
        var name = name
        ...
        val dirName = DataManager.createDateTimePrefix() + "_" + DataManager.createUniqueId()
...
//            name = String.format(Locale.getDefault(), "%1$te %1$tb %1$tY %1$tR", new Date());
        ...
        val node = if (NodesManager.getQuicklyNode() != null) NodesManager.getQuicklyNode() else TetroidXml.ROOT_NODE
        val record = TetroidRecord(
            false, id,
            name, null, null, url,
            Date(), dirName, TetroidRecord.DEF_FILE_NAME, node
        ...
        val dirPath = RecordsManager.getPathToRecordFolderInTrash(context, record)
        if (RecordsManager.checkRecordFolder(context, dirPath, true) <= 0) {
            return null
        }
        // создаем файл записи (пустой)
        ...
        if (!TextUtils.isEmpty(text)) {
            if (RecordsManager.saveRecordHtmlText(context, record, text)) {
                record.setIsNew(false)
            } else {
     ...
     * @return
     */
    fun editRecordFields(context: Context, record: TetroidRecord?, name: String?, tagsString: String?,
        ...
        if (isTemp) {
            // вырезаем уникальную приставку в имени каталога
            val dirNameInBase = oldDirName.substring(DataManager.PREFIX_DATE_TIME_FORMAT.length + 1)
            // перемещаем каталог записи из корзины
            if (moveRecordFolder(
                    context,
                    record,
                    RecordsManager.getPathToRecordFolderInTrash(context, record),
                    DataManager.getStoragePathBase(),
                    dirNameInBase
                ) <= 0
            ) {
                return false
        ...
        if (saveStorage(context)) {
            if (oldTagsString == null && tagsString != null
                ...
                mTagsParser.deleteRecordTags(record)
            ...
            mCryptInteractor.cryptRecordFiles(context, record, oldIsCrypted, crypted)
        } else {
            if (isTemp) {
            ...
            if (isTemp) {
                moveRecordFolder(
                    context,
                    record,
                    RecordsManager.getPathToRecordFolderInBase(record),
     ...
     * -1 - ошибка (отсутствует каталог записи)
     */
    fun deleteRecord(context: Context, record: TetroidRecord?, withoutDir: Boolean): Int {
     ...
     * -1 - ошибка (отсутствует каталог записи)
     */
    fun cutRecord(context: Context, record: TetroidRecord?, withoutDir: Boolean): Int {
     ...
     * @return
     */
    fun insertRecord(context: Context, srcRecord: TetroidRecord?, isCutted: Boolean, node: TetroidNode?, withoutDir: Boolean): Int {
        ...
        if (!withoutDir) {
            srcDirPath =
                if (isCutted) RecordsManager.getPathToRecordFolderInTrash(context, srcRecord) else RecordsManager.getPathToRecordFolderInBase(srcRecord)
            val dirRes = RecordsManager.checkRecordFolder(context, srcDirPath, false)
            srcDir = if (dirRes > 0) {
                File(srcDirPath)
            } else {
...
//                cryptOrDecryptFile(recordFile, srcRecord.isCrypted(), crypted);
                mCryptInteractor.cryptRecordFiles(context, record, srcRecord.isCrypted, crypted)
            }
        } else {
...
//                        LogManager.log(context.getString(R.string.log_error_del_record_dir) + destDirPath, LogManager.Types.ERROR);
                        TetroidLog.logOperError(
                            context, TetroidLog.Objs.RECORD_DIR, TetroidLog.Opers.DELETE,
                            ": $destDirPath", false, -1
                        )
                        return 0
                    }
     ...
     */
    private fun deleteRecord(context: Context, record: TetroidRecord?, withoutDir: Boolean, movePath: String, isCutting: Boolean): Int {
        if (record == null) {
...
//            dirPath = (record.isTemp()) ? getPathToRecordFolderInTrash(context, record) : getPathToRecordFolderInBase(record);
            ...
            val dirRes = RecordsManager.checkRecordFolder(context, dirPath, false)
            if (dirRes <= 0) {
                return dirRes
            }
        }
     ...
     * @return
     */
    protected fun moveOrDeleteRecordFolder(context: Context, record: TetroidRecord?, dirPath: String?, movePath: String?): Int {
        if (record == null || dirPath == null) return 0
        return if (movePath == null) {
            ...
            val newDirName = DataManager.createDateTimePrefix() + "_" + record.dirName
     ...
     * @return
     */
    private fun moveRecordFolder(context: Context, record: TetroidRecord?, srcPath: String, destPath: String, newDirName: String?): Int {
        if (record == null) {
            return 0
        }
        val res = mDataInteractor.moveFile(context, srcPath, destPath, newDirName)
    ...
    fun openRecordFolder(context: Context?, record: TetroidRecord): Boolean {
        ...
        val fileFullName = getPathToRecordFolderInBase(record)
        if (!mInteractionInteractor.openFile(context, File(fileFullName))) {
    ...
    fun getEditedDate(context: Context?, record: TetroidRecord?): Date? {
        ...
        val fileName = RecordsManager.getPathToFileInRecordFolderInBase(record, record.fileName)
        return DataManager.getFileModifiedDate(context, fileName)
    }
    ...
    fun getUriToRecordFolder(record: TetroidRecord): String? {
        return if (DataManager.isLoaded()) DataManager.getStoragePathBaseUri().toString() + DataManager.SEPAR + record.dirName + DataManager.SEPAR else null
    }
...
//                : null;
        return (if (record.isTemp) DataManager.getTrashPathBaseUri(context) else DataManager.getStoragePathBaseUri()).toString() + DataManager.SEPAR + record.dirName + DataManager.SEPAR
    }
    ...
    fun getPathToRecordFolderInBase(record: TetroidRecord): String {
        return DataManager.getStoragePathBase() + DataManager.SEPAR + record.dirName
    }
    ...
    fun getPathToRecordFolder(context: Context?, record: TetroidRecord): String {
        return (if (record.isTemp) SettingsManager.getTrashPath(context) else DataManager.getStoragePathBase()) + DataManager.SEPAR + record.dirName
    }

    fun getPathToRecordFolderInTrash(context: Context?, record: TetroidRecord): String? {
        return SettingsManager.getTrashPath(context) + DataManager.SEPAR + record.dirName
    }

    @Deprecated("")
    fun getPathToFileInRecordFolderInBase(record: TetroidRecord, fileName: String): String? {
        return getPathToRecordFolderInBase(record) + DataManager.SEPAR + fileName
    }

    fun getPathToFileInRecordFolder(context: Context?, record: TetroidRecord, fileName: String): String? {
        return getPathToRecordFolder(context, record) + DataManager.SEPAR + fileName
    }
...
//                : findRecordInHierarchy(Instance.getRootNodes(), id, comparator);
    ...
    fun findRecordInHierarchy(fieldValue: String?, comparator: TetroidRecordComparator): TetroidRecord? {
        ...
        if (findRecord(TetroidXml.ROOT_NODE.records, fieldValue, comparator).also { found = it } != null) return found
        return if (App.IsLoadedFavoritesOnly) findRecord(
            FavoritesManager.getFavoritesRecords(),
            fieldValue,
            comparator
        ) else findRecordInHierarchy(DataManager.getRootNodes(), fieldValue, comparator)
    }
    ...
    fun findRecordInHierarchy(nodes: List<TetroidNode>?, fieldValue: String?, comparator: TetroidRecordComparator?): TetroidRecord? {
        ...
        var found: TetroidRecord?
        for (node in nodes) {
            if (findRecord(node.records, fieldValue, comparator).also { found = it } != null) return found
            if (node.isExpandable) {
                if (findRecordInHierarchy(node.subNodes, fieldValue, comparator).also { found = it } != null) return found
            }
        }
        return null
    }
    ...
    fun saveStorage(context: Context) = mStorageInteractor.saveStorage(context)
    ...
    fun getCrypter() = mCryptInteractor.mCrypter
}